...
async def sync_multiple_sites_sequential(site_ids: List[int], days: int = 7, sync_mode: str = "skip") -> Dict[str, Any]:
    """
    ...
    """
    print(f"🔄 Starting SEQUENTIAL sync for {len(site_ids)} sites ({days} days each, mode: {sync_mode})")
        ...
        for i, site_id in enumerate(site_ids, 1):
            ...
            try:
                ...
                sync_cmd = [
                    ...
                    "sync", str(site_id), str(days), sync_mode
                ]
    ...
    print(f"\n" + "="*60)
    print(f"                SEQUENTIAL SYNC SUMMARY")
    print(f"="*60)
    print(f"Total sites processed: {len(site_ids)}")
    print(f"Successful syncs: {successful}/{len(site_ids)} ({successful/len(site_ids)*100:.1f}%)")
    ...
    for result in results:
        ...
        print(f"  {status} {result['site_name']} (ID: {result['site_id']}): "
...
async def main():
    ...
    print(f"🚀 Multi-Site Sequential Sync")
    print(f"Sites: {site_ids}")
    print(f"Days per site: {days}")
    print(f"Sync mode: {sync_mode}")
    print(f"⚠️  Using sequential processing (GSC API requirement)")
