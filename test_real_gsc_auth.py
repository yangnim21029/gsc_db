    ...
    try:
        ...
        if rows:
            ...
            for i, row in enumerate(rows[:3]):
                ...
                print(f"   {i+1}. Query: {keys[0] if keys else 'N/A'}, Clicks: {clicks}, Impressions: {impressions}")
        else:
        ...
        if e.resp.status == 403:
            print(f"❌ Access denied - site may not be verified or no permission")
        elif e.resp.status == 400:
            print(f"❌ Bad request - check date range or parameters")
        else:
...
async def test_concurrent_queries(service, sites, max_concurrent=3):
    ...
    for site in test_sites:
        ...
        print(f"     {site['siteUrl']}: {'✅' if result['success'] else '❌'} ({result['rows']} rows)")
    ...
    for result in concurrent_results:
        print(f"     {result['site']}: {'✅' if result['success'] else '❌'} ({result['rows']} rows)")
