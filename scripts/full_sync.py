...
from src.jobs.bulk_data_synchronizer import run_sync  # noqa: E402
from src.services.database import SyncMode  # noqa: E402
...
def main():
    """主函式，用於執行全站數據同步任務。"""
    console.print(
        "[bold green]🚀 開始全站數據同步任務...[/bold green]",
    ...
    try:
        ...
        console.print(
            f"   [dim]將精確同步 {len(SITES_TO_ENSURE)} 個在 SITES_TO_ENSURE 列表中的站點...[/dim]"
        )
        ...
        console.print(
            "\n[bold green]🎉 全站數據同步任務圓滿結束！[/bold green]",
            style="bold white on blue",
        )
