...
class GSCClient:
    ...
    def sync_site_data(self, site_url: str, start_date: str,
                       end_date: str) -> Dict[str, Any]:
        """同步站點數據 - 調用 enhanced 版本"""
        return self.sync_site_data_enhanced(site_url, start_date, end_date)

    def sync_multiple_sites(
            self, sites: List[str], start_date: str, end_date: str, max_workers: int = 5) -> List[Dict[str, Any]]:
        """使用線程池並發同步多個站點的數據"""
        results = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 創建 future 到 site_url 的映射
            future_to_site = {
                executor.submit(self.sync_site_data_enhanced, site_url, start_date, end_date): site_url
            ...
            for future in concurrent.futures.as_completed(future_to_site):
                ...
                try:
                    ...
                    logger.info(f"Successfully synced {site_url}")
                except Exception as e:
    ...
    def sync_site_data_enhanced(
            self, site_url: str, start_date: str, end_date: str) -> Dict[str, Any]:
        """增強版站點數據同步"""
        try:
            # 獲取或創建站點記錄
            ...
            # 同步關鍵字數據
            keyword_result = self._sync_keyword_data(
                site, site_url, start_date, end_date)

            # 同步頁面數據
            page_result = self._sync_page_data(
                site, site_url, start_date, end_date)
    ...
    def _sync_keyword_data(self,
        ...
        # 獲取搜索數據
        search_data = self._fetch_search_data_by_devices(site_url, start_date, end_date)
        
        # 如果數據量不足，添加頁面維度數據
        if len(search_data) < 15000:
            search_data = self._add_page_dimension_data(search_data, site_url, start_date, end_date)
        
        # 處理和合併數據
        keywords_added, rankings = self._process_search_data(search_data, site['id'])
    ...
    def _fetch_search_data_by_devices(self, site_url: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """按設備維度獲取搜索數據"""
        all_search_data = []
    ...
    def _add_page_dimension_data(self, existing_data: List[Dict[str, Any]], 
                                site_url: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """添加頁面維度數據，避免重複"""
        ...
        # 創建現有數據的鍵集合
        existing_keys = self._create_existing_keys_set(existing_data)
        ...
        logger.info(f"Total after page dimension: {len(existing_data)} records")
        return existing_data

    def _create_existing_keys_set(self, search_data: List[Dict[str, Any]]) -> set:
        """創建現有數據的鍵集合"""
        existing_keys = set()
    ...
    def _process_search_data(self, search_data: List[Dict[str, Any]], site_id: int) -> tuple:
        """處理搜索數據，返回關鍵字集合和排名數據"""
        keywords_added = set()
        ranking_map = {}  # 用於合併同一關鍵字的多條記錄

        for row in search_data:
            processed_data = self._extract_row_data(row)
            if processed_data:
                date, query, device, page = processed_data
                
                # 添加關鍵字
                keyword_id = self.database.add_keyword(query, site_id)
                if keyword_id:
                    keywords_added.add(query)
                    # 合併同一天同一關鍵字的數據
                    self._merge_ranking_data(ranking_map, row, site_id, keyword_id, date, query, device, page)

        rankings = list(ranking_map.values())
        return keywords_added, rankings

    def _extract_row_data(self, row: Dict[str, Any]) -> Optional[tuple]:
        """從行數據中提取關鍵信息"""
        keys = row.get('keys', [])
        if len(keys) < 2:
            return None
        ...
        return date, query, device, page

    def _merge_ranking_data(self, ranking_map: Dict[str, Any], row: Dict[str, Any], 
                           site_id: int, keyword_id: int, date: str, query: str, device: str, page: str):
        """合併排名數據"""
        ...
        if key in ranking_map:
            ...
            existing['position'] = min(
                existing['position'], row.get('position', 999)
            ) if existing['position'] > 0 else row.get('position', 0)
    ...
    def _sync_page_data(self,
        ...
        # 處理頁面數據
        pages = self._process_page_data(page_data, site['id'])
    ...
    def _process_page_data(self, page_data: List[Dict[str, Any]], site_id: int) -> List[Dict[str, Any]]:
        """處理頁面數據"""
        pages = []
        for row in page_data:
            page_record = self._extract_page_record(row, site_id)
            if page_record:
                pages.append(page_record)
        return pages

    def _extract_page_record(self, row: Dict[str, Any], site_id: int) -> Optional[Dict[str, Any]]:
        """從行數據中提取頁面記錄"""
        keys = row.get('keys', [])
        if len(keys) < 2:
            return None
