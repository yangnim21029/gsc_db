...
class GSCVisualizer:
    ...
    def plot_daily_trends(self, days=30, save_path=None):
        ...
        ax_main.plot(
            ...
            markeredgewidth=2,
            markeredgecolor=DUOLINGO_COLORS['green'],
            label='點擊量',
            zorder=3)

        # 曝光量區域圖（右軸）
        ax_main2.fill_between(
            df['date'],
            df['total_impressions'],
            color=DUOLINGO_COLORS['blue'],
            alpha=0.3,
            label='曝光量')
        ax_main2.plot(df['date'], df['total_impressions'],
        ...
        ax4.text(
            ...
            bbox=dict(
                ...
                facecolor=UBER_COLORS['light'],
                edgecolor=UBER_COLORS['secondary'],
                linewidth=2),
            fontweight='bold',
            color=UBER_COLORS['dark'])
    ...
    def plot_top_keywords(self, limit=20, days=7, save_path=None):
        ...
        for i, (bar, value) in enumerate(
                zip(bars1, top_clicks['total_clicks'])):
            ax1.text(bar.get_width() +
                     max(top_clicks['total_clicks']) *
                     0.01, bar.get_y() +
                     bar.get_height() /
                     2, f'{int(value)}', va='center', fontweight='bold', fontsize=9)
        ...
        for i, (bar, value) in enumerate(
                zip(bars2, top_impressions['total_impressions'])):
            ax2.text(bar.get_width() +
                     max(top_impressions['total_impressions']) *
                     0.01, bar.get_y() +
                     bar.get_height() /
                     2, f'{int(value):,}', va='center', fontweight='bold', fontsize=9)
        ...
        ax_stats.text(
            ...
            bbox=dict(
                boxstyle="round,pad=0.5",
                facecolor=DUOLINGO_COLORS['blue'],
                alpha=0.1,
                edgecolor=DUOLINGO_COLORS['blue'],
                linewidth=2),
            color=UBER_COLORS['dark'])
    ...
    def plot_page_performance(self, limit=15, days=7, save_path=None):
        ...
        ax2.barh(
            y_pos,
            df_impressions['total_impressions'],
            color='#8c564b',
            alpha=0.8)
        ax2.set_yticks(y_pos)
        ax2.set_yticklabels(
            df_impressions['page_short'],
            fontsize=8,
            ha='right')
    ...
    def data_summary(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT SUM(clicks), SUM(impressions) FROM daily_rankings")
