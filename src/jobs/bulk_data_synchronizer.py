...
def _handle_resume_state(
    sites_to_sync: List[Dict[str, Any]], date_list: List[str], resume: bool
) -> tuple[int, int]:
    """處理恢復狀態邏輯"""
    start_site_index, start_date_index = 0, 0

    if not resume:
        return start_site_index, start_date_index
    ...
    try:
        ...
        if last_site_id is not None:
            start_site_index = next(
                i for i, s in enumerate(sites_to_sync) if s["id"] == last_site_id
            )
        ...
        if start_site_index < len(sites_to_sync):
            ...
            logger.info(
                f"[bold yellow]🔄 任務恢復中...[/bold yellow] "
                f"將從站點 '{resume_site_name}' 的日期 '{resume_date}' 開始。"
            )
        else:
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                logger.error(
                    f"站點 [bold red]{site_name}[/bold red] "
                    f"日期 [bold red]{date}[/bold red] 在 {retries} 次重試後最終失敗。"
                )
...
def run_sync(
    ...
    start_site_index, start_date_index = _handle_resume_state(sites_to_sync, date_list, resume)

    # 檢查是否已完成
    if start_site_index == -1:
        return
    ...
    try:
        with progress:
            task = progress.add_task(
                "[green]同步進度...", total=total_days_to_sync, completed=completed_days
            )
...
def _get_sync_dates(start_date: Optional[str], end_date: Optional[str], days: int) -> List[str]:
    ...
    try:
        ...
        start_dt = (
            ...
            if start_date
            else end_dt - timedelta(days=days - 1)
        )
    except ValueError:
...
def _get_sites_to_sync(
    database: Database, site_url: Optional[str], site_id: Optional[int], all_sites: bool
) -> List[Dict[str, Any]]:
    """根據輸入參數獲取要同步的站點列表"""
    sites_to_sync: List[Dict[str, Any]] = []
