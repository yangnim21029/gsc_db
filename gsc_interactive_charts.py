...
class InteractiveBubbleChart:
    ...
    def get_keyword_bubble_data(self, days=30, limit=50):
        ...
        try:
            ...
            # 清理關鍵字名稱
            df['query_clean'] = df['query'].apply(lambda x: x[:50] + '...' if len(x) > 50 else x)
    ...
    def create_keyword_bubble_chart(self, days=30, limit=50, save_path=None):
        ...
        # 優秀表現象限 (左上角)
        fig.add_annotation(x=df['avg_position'].min() + x_range*0.1, 
        ...
        # 潛力關鍵字象限 (右上角)  
        fig.add_annotation(x=df['avg_position'].max() - x_range*0.15, 
                          y=df['ctr_percent'].max() - y_range*0.1,
        ...
        if save_path:
            ...
            self.save_bubble_with_analysis(fig, quadrant_analysis, save_path, "關鍵字")
    ...
    def generate_quadrant_analysis(self, df):
        ...
        for quadrant in quadrants:
            quadrants[quadrant] = sorted(quadrants[quadrant], 
                                       key=lambda x: x['clicks'], reverse=True)[:10]
    ...
    def save_bubble_with_analysis(self, fig, analysis, save_path, chart_type):
        ...
        data_json = raw_data.to_json(orient='records') if raw_data is not None else "[]"
...
"""
    ...
    def generate_analysis_html(self, analysis, chart_type):
        ...
        for i, (quadrant_name, items) in enumerate(analysis["quadrants"].items()):
            html_parts.append(f'<div class="quadrant {quadrant_classes[i]}">')
            html_parts.append(f'<h3>{quadrant_icons[i]} {quadrant_name} ({len(items)})</h3>')
            
            if not items:
                html_parts.append('<p style="color: #999; font-style: italic;">暫無數據</p>')
            else:
                for item in items:
                    html_parts.append(f'''
    ...
    def create_multi_site_comparison(self, days=30, save_path=None):
        ...
        if save_path:
            ...
            # 將氣泡圖和四象限分析結合到一個 HTML 頁面
            self.save_bubble_with_analysis(fig, quadrant_analysis, save_path, "多站點對比")
...
def main():
    ...
    if args.type == 'keywords':
        bubble_chart.create_keyword_bubble_chart(args.days, args.limit, args.save)
