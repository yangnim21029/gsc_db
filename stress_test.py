...
async def test_ranking_data_concurrent(client: httpx.AsyncClient, user_id: int, test_data: dict) -> Dict[str, Any]:
...
async def run_concurrent_stress_test(concurrent_users: int) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient(
        limits=httpx.Limits(
            max_keepalive_connections=concurrent_users * 2,
            max_connections=concurrent_users * 4
        ),
        ...
        metrics = {
            ...
            "avg_response_time": round(sum(r["duration_ms"] for r in successful) / len(successful), 2) if successful else 0,
            ...
            "avg_records_per_request": round(sum(r["total_records"] for r in successful) / len(successful), 1) if successful else 0
        }
        
        # Print summary
        print(f"   ✅ {metrics['successful_requests']}/{metrics['total_requests']} requests successful ({metrics['success_rate']}%)")
...
async def run_scalability_test():
    ...
    print(f"\n📈 Performance Summary:")
    print(f"{'Users':<8} {'Success%':<10} {'RPS':<8} {'Avg(ms)':<10} {'Min(ms)':<10} {'Max(ms)':<10}")
    print("-" * 66)
    ...
    for users in concurrency_levels:
        ...
        print(f"{users:<8} {metrics['success_rate']:<10} {metrics['requests_per_second']:<8} "
    ...
    if last_metrics["success_rate"] < 90:
        ...
        print(f"   - Consider implementing connection pooling or caching")
    elif last_metrics["avg_response_time"] > 1000:
        print(f"   - ⚠️  Response times degrading at high concurrency")
        print(f"   - Database query optimization recommended")
    else:
    ...
    if high_concurrency_metrics["avg_response_time"] > 500:
        print(f"   - 🔍 Potential database bottleneck detected")
        print(f"   - Consider implementing Redis caching")
        print(f"   - Monitor SQLite lock contention")
