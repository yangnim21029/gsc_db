...
class InteractiveVisualizer:
    ...
    def plot_trends(self, df: pd.DataFrame, title: str) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("排名趨勢", "點擊趨勢", "曝光趨勢", "點擊率(CTR)趨勢"),
        ...
        fig.add_trace(
            go.Scatter(
                x=df["date"],
                y=df["position"].astype(float).round(2),
                name="排名",
                yaxis="y1",
            ),
            row=1,
        ...
        fig.add_trace(
            go.Scatter(x=df["date"], y=df["clicks"].astype(int), name="點擊", yaxis="y2"),
            row=1,
            col=2,
        )
        fig.add_trace(
            go.Scatter(x=df["date"], y=df["impressions"].astype(int), name="曝光", yaxis="y3"),
            row=2,
            col=1,
        )
        fig.add_trace(
            go.Scatter(x=df["date"], y=df["ctr"].astype(float).round(4), name="CTR", yaxis="y4"),
            row=2,
            col=2,
        )
