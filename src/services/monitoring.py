...
query_histogram = Histogram(
    "gsc_query_duration_seconds", "Query duration in seconds", ["query_type"]
)

active_syncs = Gauge("gsc_active_syncs", "Number of active sync operations")

api_request_counter = Counter(
    "gsc_api_requests_total", "Total API requests", ["endpoint", "method", "status_code"]
)

cache_hit_counter = Counter(
    "gsc_cache_hits_total", "Cache hit/miss counter", ["cache_level", "hit"]
)

db_connection_gauge = Gauge("gsc_db_connections", "Number of database connections", ["db_type"])
...
class MonitoredService:
    ...
    def __init__(self, service_name: str):
        ...
        self.request_counter = self.meter.create_counter(
            f"{service_name}_requests_total", description=f"Total requests to {service_name}"
        )

        self.error_counter = self.meter.create_counter(
            f"{service_name}_errors_total", description=f"Total errors in {service_name}"
        )

        self.duration_histogram = self.meter.create_histogram(
            f"{service_name}_duration_seconds", description=f"Request duration for {service_name}"
        )
