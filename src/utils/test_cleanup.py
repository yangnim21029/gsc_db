...
class TestDataCleaner:
    ...
    def clean_test_site_data(self, site_id: int = 3, days_to_keep: int = 0) -> dict[str, int]:
        ...
        try:
            ...
            # Delete records
            cursor.execute(
                f"DELETE FROM gsc_performance_data WHERE site_id = ? {date_condition}",
                (site_id,)
            )
            
            cursor.execute(
                f"DELETE FROM hourly_rankings WHERE site_id = ? {date_condition}",
                (site_id,)
            )
    ...
    def clean_future_data(self) -> dict[str, int]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM gsc_performance_data WHERE date > ?",
                (today,)
            )
            performance_count = cursor.fetchone()[0]
            
            cursor.execute(
                "SELECT COUNT(*) FROM hourly_rankings WHERE date > ?",
                (today,)
            )
            hourly_count = cursor.fetchone()[0]
            ...
            # Delete future data
            cursor.execute(
                "DELETE FROM gsc_performance_data WHERE date > ?",
                (today,)
            )
            
            cursor.execute(
                "DELETE FROM hourly_rankings WHERE date > ?",
                (today,)
            )
