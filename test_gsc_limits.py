...
class GSCAPITester:
    ...
    async def simulate_gsc_request(self, site_url: str, date: str, request_id: int) -> Dict[str, Any]:
    ...
    async def test_concurrent_requests(self, site_urls: List[str], days: int = 3) -> Dict[str, Any]:
        ...
        print(f"  ✅ Concurrent: {successful}/{len(processed_results)} successful in {total_time:.2f}s")
    ...
    async def test_batch_concurrent_requests(self, site_urls: List[str], days: int = 3, batch_size: int = 3) -> Dict[str, Any]:
...
async def main():
    ...
    for result in results:
        ...
        req_per_sec = round(result['total_requests'] / result['total_time'], 1) if result['total_time'] > 0 else 0
    ...
    concurrent_rate_limits = sum(1 for r in concurrent_result['results'] if r.get('status_code') == 429)
    if concurrent_rate_limits > 0:
        print(f"   ⚠️  {concurrent_rate_limits} requests hit rate limits in concurrent mode")
        print(f"   - GSC API likely has concurrent request limits")
        print(f"   - Sequential or batched approach recommended")
    else:
        print(f"   ✅ No rate limiting detected in test")
    ...
    if concurrent_time > 0:
        ...
        if speedup > 2:
            print(f"   ✅ Significant benefit from concurrency")
        elif speedup > 1.2:
            print(f"   ⚖️  Moderate benefit from concurrency")
        else:
            print(f"   ⚠️  Limited benefit from concurrency")
    
    # Recommendations
    print(f"\n💡 Recommendations for GSC Sync:")
    
    best_success_rate = max(r['successful']/r['total_requests'] for r in results)
    best_mode = next(r for r in results if r['successful']/r['total_requests'] == best_success_rate)
    
    print(f"   - Best approach: {best_mode['mode'].capitalize()}")
    print(f"   - Success rate: {best_mode['successful']}/{best_mode['total_requests']}")
    
    if concurrent_rate_limits > 0:
        print(f"   - Use sequential sync or small batches (2-3 sites)")
        print(f"   - Add delays between requests (200-500ms)")
        print(f"   - Implement exponential backoff for 429 errors")
    else:
        print(f"   - Concurrent sync appears feasible")
        print(f"   - Monitor for rate limits in production")
