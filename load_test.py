...
async def test_ranking_data_api(client: httpx.AsyncClient, user_id: int) -> dict[str, Any]:
    ...
    try:
        response = await client.post(
            "http://localhost:8000/api/v1/analytics/ranking-data", json=payload, timeout=30.0
        )
...
async def run_concurrent_test(
    endpoint_name: str, test_func, num_users: int = 10
) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(
        ...
        tasks = [test_func(client, user_id) for user_id in range(1, num_users + 1)]
...
async def main():
    ...
    csv_results = await run_concurrent_test("csv-export", test_csv_export_api, num_users=10)
