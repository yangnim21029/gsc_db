...
from ..utils.matplotlib_utils import DUOLINGO_COLORS, set_chinese_font
...
class GSCVisualizer:
    ...
    def plot_top_keywords(
        ...
        axes[0, 0].barh(
            top_clicks["query"],
            top_clicks["total_clicks"],
            color=DUOLINGO_COLORS["blue"],
            zorder=3,
        )
        axes[0, 0].set_title("點擊數 Top 10", fontweight="bold")

        top_impressions = df.nlargest(10, "total_impressions").sort_values(
            "total_impressions", ascending=True
        )
        axes[0, 1].barh(
            top_impressions["query"],
        ...
        top_position = (
            df[df["avg_position"].notna()]
            .nsmallest(10, "avg_position")
            .sort_values("avg_position", ascending=False)
        )
        axes[1, 0].barh(
            top_position["query"],
        ...
        top_ctr = (
            df[df["avg_ctr"].notna()].nlargest(10, "avg_ctr").sort_values("avg_ctr", ascending=True)
        )
        axes[1, 1].barh(
            top_ctr["query"],
            top_ctr["avg_ctr"] * 100,
    ...
    def plot_page_performance(
        ...
        top_impressions = df.nlargest(limit, "total_impressions").sort_values(
            "total_impressions", ascending=True
        )
        axes[1].barh(
            top_impressions["page_short"],
            top_impressions["total_impressions"],
            color=DUOLINGO_COLORS["green"],
            zorder=3,
        )
...
def build_report(
    ...
    if include_plots:
        ...
        # 生成並保存圖表
        try:
            visualizer.plot_daily_trends(
                site_id, days, save_path=str(save_dir / "daily_trends.png")
            )
            visualizer.plot_top_keywords(
                site_id, days=days, save_path=str(save_dir / "top_keywords.png")
            )
            visualizer.plot_page_performance(
                site_id, days=days, save_path=str(save_dir / "page_performance.png")
            )
        except Exception as e:
