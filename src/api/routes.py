...
async def list_sites(
    db: HybridDataStore,
    active_only: Annotated[bool, Parameter(default=True)]
) -> list[Site]:
    """List all sites."""
...
async def get_performance_trends(
    db: HybridDataStore,
    site_id: Annotated[Optional[int], Parameter(default=None)],
    hostname: Annotated[Optional[str], Parameter(default=None)],
    ...
    for row in df.to_dicts():
        trend_data.append(PerformanceTrendData(
            date=str(row["date"]),
            ...
            clicks_wow_change=float(row["clicks_wow_change"]) if row.get("clicks_wow_change") else None,
