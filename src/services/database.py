...
class Database:
    ...
    def save_ranking_data(self, rankings: List[Dict[str, Any]]) -> int:
        ...
        data_to_insert = [
            (
                r['site_id'], r['keyword_id'], r['date'], r['query'],
                r.get('position'), r.get('clicks', 0), r.get('impressions', 0),
                r.get('ctr', 0), r.get('page'), r.get('country', 'TWN'),
                r.get('device', 'ALL')
            ) for r in rankings
        ]
        ...
        with self.get_connection() as conn:
            try:
                ...
                logger.error(f"Failed to save ranking data in batch: {e}")
                conn.rollback()
                return 0
    ...
    def save_page_data(self, page_data_list: List[Dict[str, Any]]) -> int:
        ...
        data_to_insert = [
            (
                p['site_id'], p['page'], p['date'],
                p.get('clicks', 0), p.get('impressions', 0), p.get('ctr', 0),
                p.get('position', 0), p.get('country', 'TWN'), p.get('device', 'ALL')
        ...
        with self.get_connection() as conn:
            try:
                ...
                logger.error(f"Failed to save page data in batch: {e}")
                conn.rollback()
                return 0
