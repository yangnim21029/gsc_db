...
class GSCClient:
    ...
    def sync_multiple_sites(
            self, sites: List[str], start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """同步多個站點的數據"""
        results = []
        for site_url in sites:
            try:
                result = self.sync_site_data_enhanced(
    ...
    def _sync_keyword_data(self,
        ...
        if len(all_search_data) < 15000:  # 如果少於預期，再查一次包含頁面的數據
            ...
            logger.info(
                f"Total after page dimension: {len(all_search_data)} records")
        ...
        for row in search_data:
            ...
            if len(keys) >= 2:
                ...
                page = keys[3] if len(keys) > 3 else keys[2] if len(
                    keys) == 3 and device == 'ALL' else None
                ...
                if key in ranking_map:
                    ...
                    existing['position'] = min(
                        existing['position'], row.get(
                            'position', 999)) if existing['position'] > 0 else row.get(
                        'position', 0)
                    if existing['impressions'] > 0:
                        existing['ctr'] = existing['clicks'] / \
