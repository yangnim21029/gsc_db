...
def main():
    ...
    # 認證
    auth_parser = subparsers.add_parser('auth', help='進行 GSC API 認證')
    ...
    # 進度監控 (簡化版)
    progress_parser = subparsers.add_parser('progress', help='顯示最近任務')
    ...
    hourly_summary_parser = subparsers.add_parser('hourly-summary', help='顯示每小時數據總結')
    hourly_summary_parser.add_argument('--site-id', type=int, help='站點 ID')
    hourly_summary_parser.add_argument('--date', help='日期 (可選)')
    
    hourly_coverage_parser = subparsers.add_parser('hourly-coverage', help='顯示每小時數據覆蓋情況')
    hourly_coverage_parser.add_argument('--site-id', type=int, help='站點 ID')
    
    # API 狀態
    api_parser = subparsers.add_parser('api-status', help='顯示 API 使用狀態')
    
    # 日誌查看
    logs_parser = subparsers.add_parser('logs', help='查看同步日誌')
    logs_parser.add_argument('--lines', type=int, default=50, help='顯示行數 (默認50行)')
    logs_parser.add_argument('--error-only', action='store_true', help='只顯示錯誤日誌')
        ...
        with database.get_connection() as conn:
            if site_id:
                ...
                if result:
                    ...
                    print(f"數據範圍：{site['earliest_date']} 至 {site['latest_date']}")
...
def print_coverage_table(sites: list):
    ...
    for site in sites:
        ...
        print(f"{site['id']:<4} {site['name'][:19]:<20} {site['earliest_date'] or 'N/A':<12} "
    ...
    try:
        ...
        if args.all_sites:
            ...
            for site in sites:
                ...
                print(f"✅ 完成：關鍵字 {result['keyword_count']} 條，頁面 {result['page_count']} 條")
        
        elif args.site_url:
            # 同步指定 URL 的站點
            print(f"🔄 同步站點：{args.site_url}")
            result = gsc_client.sync_site_data_enhanced(args.site_url, start_date, end_date)
            print(f"✅ 完成：關鍵字 {result['keyword_count']} 條，頁面 {result['page_count']} 條")
...
def show_progress():
    ...
    try:
        ...
        for task in tasks:
            ...
            print(f"{task['id']:<4} {site_name:<20} {task['task_type']:<15} "
...
def show_logs(args):
    ...
    try:
        ...
        if args.error_only:
            error_lines = []
            for line in lines:
                if any(keyword in line for keyword in ['ERROR', 'Failed', 'failed', '❌']):
        ...
        if args.error_only:
            ...
            error_count = len([line for line in lines if any(keyword in line for keyword in ['ERROR', 'Failed', 'failed'])])
...
def plot_clicks_trend(conn, site_id, start_date, end_date, save_path=None):
    ...
    ax1.set_title(f'每日點擊量趨勢 (站點 ID: {site_id})', fontsize=14, fontweight='bold')
