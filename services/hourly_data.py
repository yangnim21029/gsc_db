...
class HourlyDataHandler:
    ...
    def get_hourly_data_batch(self, site_url: str, start_date: str, end_date: str,
        ...
        try:
            ...
            while max_total_rows is None or len(all_rows) < max_total_rows:
                ...
                if len(rows) < max_rows_per_request and len(rows) < 1000:
                    logger.info(f"Received only {len(rows)} hourly rows (< 1000), likely end of data")
                    break
    ...
    def sync_hourly_data(self, site_url: str, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 保存到數據庫
            logger.info(f"About to save {len(hourly_rankings)} hourly rankings (skipped {skipped_rows} rows)")
            saved_count = self.database.save_hourly_ranking_data(hourly_rankings)
            
            logger.info(f"Synced {saved_count} hourly records for {site_url} from {start_date} to {end_date}")
