...
API_BASE_URL = (
    "https://www.googleapis.com/webmasters/v3"  # Fixed: was incorrect v1 URL causing 404 errors
)
...
class ModernGSCClient:
    ...
    async def fetch_data_for_date(
        self, site_url: str, target_date: date, row_limit: int = 25000, start_row: int = 0
    ...
    async def fetch_batch(
        self, site_url: str, dates: list[date]
...
class GSCAuthManager:
    ...
    @staticmethod
    async def authenticate(credentials_path: Path, token_path: Path | None = None) -> Credentials:
