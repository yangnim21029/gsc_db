...
class HybridDataStore:
    ...
    async def add_site(self, domain: str, name: str, category: Optional[str] = None) -> int:
        """Add a new site."""
        async with self._lock:
            cursor = await self._sqlite_conn.execute(
                """
                ...
                if mode == "skip":
                    ...
                    try:
                        await conn.execute(
                            ...
                            """,
                            (
                                record.site_id, record.date, record.page, 
                                record.query, record.device, record.search_type,
                                record.clicks, record.impressions, 
                                record.ctr, record.position
                            )
                        )
                        stats["inserted"] += 1
    ...
    async def analyze_performance_trends(
        ...
        result = await loop.run_in_executor(
            None, 
            lambda: self._duck_conn.execute(query).pl()
        )
    ...
    async def get_site_by_hostname(self, hostname: str) -> Optional[Site]:
    ...
    async def export_performance_csv(self, data: list[dict]) -> str:
        ...
        if not data:
            return "url,total_clicks,total_impressions,avg_ctr,avg_position,keywords,keyword_count\n"
    ...
    async def _get_ranking_data_duckdb(
        ...
        df = await loop.run_in_executor(
            None,
            lambda: self._duck_conn.execute(query, params).pl()
        )
